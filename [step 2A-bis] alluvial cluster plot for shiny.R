#[step 2A-bis] alluvial plot for shiny
...
extract_surname <- function(author) {
  ...
  parts <- unlist(strsplit(author, " "))
  
  # Cas avec moins de 2 morceaux → on renvoie tel quel
  ...
  # Cas "Van" particulier
  if (first == "Van") {
    if (length(parts) >= 3 && substr(parts[2], 1, 1) == "D") {
      return(paste(parts[1:3], collapse = " "))  # Van Der / Van Den
    } else {
      return(paste(parts[1:2], collapse = " "))  # Van X
    }
  }
  
  # Cas "préfixes spéciaux"
  if (first %in% c("De", "Da", "Di", "Dos", "Van't")) {
    return(paste(parts[1:2], collapse = " "))
  }
  
  # Cas par défaut : juste le premier mot
  return(first)
}
...
# 1. Définissez le nom exact du cluster que vous voulez mettre en évidence
cluster_a_garder_opaque <- "Trust at First Sight" # Remplacez par le nom de votre choix

# 2. Créez DEUX colonnes distinctes pour la transparence
alluv_dt <- alluv_dt %>%
  mutate(
    # Alpha pour les strates (barres verticales)
    # Le cluster focus est à 100% opaque, les autres à 50%
    alpha_stratum = ifelse(intertemporal_name == cluster_a_garder_opaque, 1.0, 0.4),
    
    # Alpha pour les flux (liens entre les barres)
    # Le cluster focus est à 100% opaque, les autres à 20% (plus discrets)
    alpha_flow    = ifelse(intertemporal_name == cluster_a_garder_opaque, 0.5, 0.2)
  )
...
ggplot(alluv_dt, aes(x = window, y= y_alluv, stratum = intertemporal_name, alluvium = ID_Art, fill = intertemporal_name)) +
  ...
  # Utiliser la colonne "alpha_stratum" pour les barres verticales
  geom_stratum(aes(alpha = alpha_stratum), size = 1/12) +
  
  # Utiliser la colonne "alpha_flow" pour les liens
  geom_flow(aes(alpha = alpha_flow)) +
  
  # Les étiquettes restent 100% opaques pour une lisibilité maximale
  geom_label(
    stat = "stratum", 
    aes(label = cluster_label), # Pas de 'alpha' ici
    size = 2.5,
    color = "white",
    colour = "black",
    label.size = 0.5,
  ...
  # Cette fonction s'applique à TOUTES les esthétiques 'alpha' du plot
...
ggsave(paste("[", cluster_a_garder_opaque, "]", " alluvial_plot.png", sep=""), plot = last_plot(), width = 12, height = 6, dpi = 900, limitsize = FALSE)
